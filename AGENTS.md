...
- **Test Assertion Patterns**: Follow these specific patterns:
  1. For synchronous methods throwing exceptions: `Assert.That(() => method(), Throws.TypeOf<TException>())`
  2. For async methods throwing exceptions: `Assert.That(async () => await method(), Throws.TypeOf<TException>())` (keep test async)
  3. Never use `Assert.DoesNotThrow(...)`, just make the actual call directly
  4. If a test doesn't use `await`, make it `void` instead of `async Task`
  5. Don't put `async () => await` in lambdas for synchronous calls, just use `() => method()`
